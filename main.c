...
#include <errno.h>
#include <string.h>
//#define DBG printf("line:%d @ file:%s\n",__LINE__,__FILE__);
...
int handleoutput( int fd )
{
    ...
    static const char compare3[]="re you sure you want to continue connecting (yes/no)?";
    char buffer[1024];
    int ret=0;
    ...
    if( numread<0 ) {
        ...
        //printf("error:read nothing from the remote. line: %d file:%s",__LINE__,__FILE__);
        return -1;
    }
